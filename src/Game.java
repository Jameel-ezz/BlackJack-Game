...
public class Game {
    ...
    JFrame frame;
    Deck deck;
    GameTools GameComponents;
    GameTools cardComponent;
    ...
    public Game(JFrame f) {//with this constructor, we initialize the instant fields accordingly. This constructor gets a JFrame as a parameter.
        ...
        GameComponents = new GameTools(dealerHand, playerHand);
        frame = f;
        faceDown = true;
        dealerWon = true;
        roundOver = false;
    ...
    public void formGame() {//this method will help us create the background of our game.
        ...
        btTryAgain = new JButton("Try again");
        btTryAgain.setBounds(930, 180, 190, 50);
        btTryAgain.setFont(new Font("Comic Sans MS", Font.BOLD, 16));

        frame.add(btnHit); //we add the buttons the JFrame
        frame.add(btnStand);
        frame.add(btnExit);
        frame.add(btTryAgain);

        btnExit.addActionListener(new ActionListener() { //we add a action listener to the exit button.
            public void actionPerformed(ActionEvent e) {
                JOptionPane.showMessageDialog(frame, "You have left the game with " +  Main.Balance + "."); //we print out the message by getting our current balance from the Tester class.
                System.exit(0); //we exit the program.
            }
        });

        GameComponents = new GameTools(dealerHand, playerHand); //we initialize the GameComponent that will be the overall atmosphere of our game.
        GameComponents.setBounds(0, 0, 1130, 665);  //we set the bounds of the component.
        frame.add(GameComponents); //we add the component to the frame.
    ...
    public void startGame() { //this method starts the game: the cards are drawn and are printed out.
        ...
        cardComponent = new GameTools(dealerHand, playerHand); //we initialize our component which accepts two card arraylists.
